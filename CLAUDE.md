...
环境：WSL2 + Ubuntu 24.04，g++ 13.3.0（支持 C++20/23），标准定为 **C++20**。

单文件运行（最常用，开发循环）：
```bash
g++ -std=c++20 -Wall -Wextra -g <file>.cpp -o <file> && ./<file>
```
...
- **顶层 [CMakeLists.txt](CMakeLists.txt)** 用 `GLOB_RECURSE` 扫描所有阶段目录 + `projects/` + `exercises/` 下的 `.cpp`，逐个 `add_executable`。target 名 = 相对路径，其中 `/` 和 `.` 替换为 `_`（如 `01-basics/hello.cpp` → `01-basics_hello_cpp`）。新增 `.cpp` 无需改 CMake，`CONFIGURE_DEPENDS` 会自动感知。
- **学习路线分 10 个阶段目录**（`01-basics/` … `10-concurrency/`），详见 [README.md](README.md) 表格。顺序即学习路径。
- **[PROGRESS.md](PROGRESS.md)** 是进度勾选清单；`notes/` 沉淀每主题笔记并在 PROGRESS.md 登记链接；`exercises/` 放练习。`projects/` 和 `exercises/`、`notes/` 目前尚未创建。
...
[.vscode/tasks.json](.vscode/tasks.json) 提供单文件构建/运行/调试任务；[.vscode/launch.json](.vscode/launch.json) 的 `F5` 调试依赖 `gdb`（需 `sudo apt install -y gdb`）。CMake 任务默认未启用，单文件开发用 g++ 任务即可。
